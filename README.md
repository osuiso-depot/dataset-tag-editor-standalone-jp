...
## 更新履歴
- 2025/11/08: メインタブの設定が保存できないバグの修正
- 2025/11/08: 設定の追加(process閾値)
- 2025/11/08: branch表示を追加(debug用)
- 2025/11/08: CPUモード時のtorch,cudaチェックをスキップするように変更。process閾値設定を追加。
- 2025/11/07: 画像拡張子でフィルタリングして読み込み効率を改善
- 2025/11/04: cpu-onlyオプションの追加。`COMMANDLINE_ARGS`に`--cpu-only`をセットするとtorchをCPUモードで開く。
- 2025/10/30: Gradio5のエンドポイントの設定ミスによりjsが読み込まれてないバグを修正
- 2025/10/29: inbrowserオプションの追加。`COMMANDLINE_ARGS`に`--inbrowser`をセットするとアプリ起動時ブラウザで開く。
